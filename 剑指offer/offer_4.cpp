...
class Solution4
{
    ...
    bool str_dfs(char *matrix, int rows, int cols, vector<bool> &flag, string str, int k, int i, int j)
    {
        int index = i * cols + j;
...
class Solution9
{
    vector<vector<int>> PrtintZ(TreeNode *pRoot)
    {
...
class Solution28
{
    vector<vector<int>> FindCounttiunousSequence(int sum)
    {
        vector<vector<int>> res;
        int left = 1, right = 2;
        while (left < right)
...
class Solution34
{
    ...
    int InverseParse(vector<int> data)
    {
        ans = 0;
        if (data.empty())
        {
...
class Solution39
{
    int ModeThanHalfNum(vector<int> numbers)
    {
        if (numbers.empty())
        {
            return 0;
