...
class Solution7
{
    ...
    TreeNode *dfs2_tree(int *&p)
    {
        ...
        p++;
        root->left = dfs2_tree(p);
        root->right = dfs2_tree(p);
        return root;
    }
};
...
class Solution10
{
    ...
    bool aux(TreeNode *r1, TreeNode *r2)
    {
        ...
        {
            return aux(r1->left, r2->right) &&
                   aux(r1->right, r2->left);
        }
...
class Solution40
{
    vector<int> GetLaastNumbers(vector<int> inputs, int k)
    {
        vector<int> res;
        if (inputs.empty())
        {
...
class Solution50
{
    ...
    {
        return i >= 0 && i < cols && j >= 0 && j < cols && !flag[i][j];
    }
