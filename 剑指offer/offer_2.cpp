...
class Solution2
{
    ...
    TreeNode *KthNode(TreeNode *pRoot, int k)
    {
        ...
        TreeNode *reNode;
        if ((reNode = KthNode(pRoot->left, k)))
        {
            return reNode;
        }
        ...
        if ((reNode = KthNode(pRoot->right, k)))
        {
            return reNode;
        }
...
class Solution8
{
    ...
    void depth(TreeNode *root, int k)
    {
        ...
        {
            vector<int> tmp;
            print_tree.emplace_back(tmp);
        }

        print_tree[k].emplace_back(root->val);
        depth(root->left, k + 1);
...
class Solution29
{
    ...
    {
        return root ? max(TreeDepth(root->left), TreeDepth(root->left)) + 1 : 0;
    }
};
