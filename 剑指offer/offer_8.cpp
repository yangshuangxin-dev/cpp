...
class Solution3
{
    ...
    int robotDfs(int threshild, int rows, int cols, vector<bool> &flag, int i, int j)
    {
        int index = i * cols + j;
        if (i < 0 || i >= rows || j < 0 || j >= cols || getSum(i) + getSum(j) < threshild || flag[index])
        {
            return 0;
        }

        flag[index] = true;
        return 1 + robotDfs(threshild, rows, cols, flag, i + 1, j) + robotDfs(threshild, rows, cols, flag, i - 1, j) + robotDfs(threshild, rows, cols, flag, i, j + 1) + robotDfs(threshild, rows, cols, flag, i, j - 1);
    }
...
class Solution63
{
    TreeNode *ReConsturctBinaryTree(vector<int> pre, vector<int> vin)
    {
