...
class Solution3
{
    int movingCount(int threshild, int rows, int cols)
    {
        ...
        return robot_dfs(threshild, rows, cols, flag, 0, 0);
    }
...
class Solution12
{
    ListNode *deleteDuplication(ListNode *pHead)
    {
        ...
        while (last)
        {
            ...
            {
                while (last->val && last->val == last->next->val)
                {
                    auto tmp = last->next;
                    last->next = tmp->next;
                    delete tmp;
